...
% ---
\titulo{DESENVOLVIMENTO DE TECNOLOGIA ASSISTIVA FOCADO EM IHC}
...
The area of assistive technology has arisen to enable people with some form of physical or motor disability to lead an autonomous life, offering some resources, products or even methods to assist them. This work aims to create computer controls aimed at assistive technology, and HCI (Human Computer Interaction). In order to achieve this goal, we will perform usability tests with people with some kind of physical or motor deficiency that makes it difficult or even impossible to use the computer. The controls will be constructed with easy-to-handle materials present in the current market.

\vspace{\onelineskip}
\noindent
\textbf{Key-words}: Assistive Technology, Usability, Physical Disability, HCI.
\end{otherlanguage*}
\end{resumo}
% ---
...
No mercado é possível encontrar alguns controles para deficientes físicos, os quais são apresentados ao longo do trabalho, como por exemplo o \emph{Trackball}, \emph{Roller Mouse}, Teclado \emph{Tix}, entre outros.
...
\begin{itemize}
	...
	\item Definir os materiais necessários de forma que seja fácil a sua montagem.
...
A ABNT NBR ISO 9241-11 \citeonline{eeihc} define usabilidade como modo que um produto pode ser utilizado por usuários, a fim de alcançar os objetivos específicos com sucesso, eficiência e satisfação em um contexto de uso. Assim um sistema interativo deve considerar que a utilização deve ser eficaz para seu usuário alvo, levando em consideração que ele pode ou não ter familiaridade com o dispositivo, ou similar a ele. A interação com usuários deve ser fácil e intuitiva levando em conta a ergonomia e interfaces simples para o usuário.

A norma NBR ISSO/IEC 9126-1 \citeonline{eeihc} declara algumas características que um sistema com uma boa usabilidade de conter, são elas:
...
Segundo Wisner \citeonline{eeihc} ergonomia é definida como um conjunto de conhecimentos científicos relacionados ao homem e a instrumentos, máquinas e dispositivos que serão utilizados, levando em conta o máximo de conforto, segurança e eficiência.
...
Baseando-se em um estudo exploratório onde foi pesquisas principalmente sobre os temas tecnologia assistiva e IHC, sendo assim possível adquirir mais conhecimento sobre eles. Sendo um trabalho qualitativo onde a pesquisa levantou o que é necessário para a criação de controles voltados para tecnologia assistiva. Toda a pesquisa foi feita baseada em artigos, livros, e conteúdos de sites encontrados na internet.

Além das pesquisas esse trabalho também se baseou em um projeto de extensão com parceria da FUNDETEC (Fundação para o Desenvolvimento Científico e Tecnológico), desenvolvido em 2016 pelo professor orientador em conjunto com outros alunos da UTFPR.
...
Antes da montagem dos controles foram feitos alguns desenhos em 3D utilizando o programa Sketchup, como vemos na Figura \ref{img:img-20}. A Figura \ref{img:img-17} mostra como ficou a versão final \footnote{Os modelos finais que foram utilizados para corte e montagem dos teclados estão no endereço http://maverick.td.utfpr.edu.br/acessibilidade} do controle que contém 10 botões configuráveis e o controle responsável pela movimentação do \emph{mouse}.

\begin{figure}[H]
	\centering
		\includegraphics[width=0.8\textwidth]{./img/img-20.png}
		\caption{Desenhos 3D no \emph{Software} Sketchup}
		\label{img:img-20}
\end{figure}
...
\begin{figure}[H]
	\centering
		...
		\caption{Desenhos Finais em 3D no \emph{Software} Sketchup dos dois controles}
		\label{img:img-17}
\end{figure}
...
\item Botões de controles de videogame \emph{arcade}. Serão responsáveis por receber os comandos do usuário e enviá-los para o ARDUINO®.
\item Chapas de MDF \footnote{Sigla MDF, do inglês \emph{Medium Density Fiberboard}} de 6 e 15mm. As quais foram utilizadas para a construção da estrutura dos controles.
\item \emph{Software} AntiMicro, responsável por simular cada tecla dos controles como uma função para o sistema operacional.
\item \emph{Software} Sketchup, onde foi feito os desenhos 3D dos controles.
\item Maquina CNC \footnote{Sigla CNC, do inglês\emph{ Computer Numeric Control}}, os desenhos 3D foram utilizados para realizar os cortes do MDF numa máquina CNC, porém podem ser facilmente cortados com ferramentas mais simples.
...
O espaçamento entre botões, deve ser o suficiente para que ao apertar um botão o usuário não acabe pela dificuldade motora, pressionando botões indesejáveis, e acabe executando outras ações. Como pode ser visto na Imagem \ref{img:img-17}, cada botão ficou aproximadamente 5 cm de distância, da área onde é pressionado , um do outro.
...
\begin{figure}[H]
	\centering
		...
		\caption{Desenhos 3D da versão final dos ajustes de inclinação dos controles}
		\label{img:img-18}
\end{figure}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.6\textwidth]{./img/img-1.png}
...
\begin{figure}[H]
	\centering
		\includegraphics[width=0.6\textwidth]{./img/img-10.jpg}
		\caption{Controle com manche montado}
		\label{img:img-10}
\end{figure}

A ligação dos botões foi desenhada no site da tinkercad (Figura \ref{img:img-19}) \footnote{O mesmo pode ser baixado na URL: http://maverick.td.utfpr.edu.br/acessibilidade}. O resistor que foi utilizado é um simples de 650 Khoms, serão necessários 4 deles e alguns fios.
...
Funções como aumentar ou diminuir o \emph{zoom}, clique direito e esquerdo do \emph{mouse}, fechar a janela atual do sistema operacional, abrir o teclado virtual, movimentação do ponteiro do \emph{mouse}, entre outras funções são possíveis de serem configuradas.
As funções podem ser configuradas de acordo com o perfil do usuário, por exemplo, um usuário que acessa mais a internet, usaria mais as funções de:
...
Já um usuario que utiliza um \emph{software} especifico pode configurar para que os controles usem os atalhos do \emph{software}.

\chapter{Conclusão}

O desenvolvimento do presente estudo possibilitou uma análise de como é a construção de um controle voltado para tecnologias assistivas. Além disso, também permitiu conhecer um pouco mais sobre as áreas que envolvem essa construção, e como ela se aplica na sociedade.

Os testes com usuários não foram realizados, pois para isso é necessário passar pelo comitê de ética, após aprovado podem ser feitos os testes necessários. Como essa aprovação não é simples e acaba sendo demorada, não foi possível ter a aprovação até a data final de entrega do trabalho.

Para trabalhos futuros pode ser feita a separação dos botões em módulos, para que o usuário ajuste como preferir em sua mesa. Também pode ser reduzido a altura dos controles, utilizando botões com alturas menores.
...
Nesse tutorial vamos aprender a transformar a placa ARDUINO® UNO em \emph{Joystick}.

Primeiramente vamos baixar os arquivos necessários apara fazer a transformação na placa ARDUINO® UNO para \emph{Joystick}, os \emph{links} estão todos no final da postagem.

Lembrando que além dos arquivos disponibilizados para \emph{download} é necessário ter também ter o java instalado no Pc, e o \emph{software} do ARDUINO® UNO.
Vamos aos procedimentos:

1º Passo: Conectar a placa ARDUINO® UNO no Pc e verificar se os \emph{drivers} estão instalados e se a porta reconheceu a placa ARDUINO®.
...
4º Passo: Abra a pasta Arduino-\emph{big}-\emph{joystick} e faça a extração da pasta compactada Arduino-\emph{big}-\emph{joystick}.
...
5º Passo: Após ter extraído os arquivos, renomeie o arquivo Arduino-\emph{big}-\emph{joystick}.hex para UnoJoy.hex, o U e J deixem maiúsculo.
...
6º Passo: copie o arquivo UnoJoy.hex volte e abra a pasta UnoJoy, abra a pasta UnoJoy outra vez, abra a pasta UnoJoy-\emph{master}, localize e abra a pasta UnoJoy mais uma vez, agora abra a pasta ATmega8u2Code, agora abra a pasta Hex\emph{Files} cole e substitua o arquivo UnoJoy.hex.
...
7º Passo: Volte até a pasta Arduino-\emph{big}-\emph{joystick}, abra a mesma e abra a pasta \emph{sketch}\_aug19c, dentro desta pasta está a \emph{sketch}  para fazer a programação no ARDUINO® UNO, abra a \emph{sketch} e execute dando dois cliques.
...
8º Passo: Com a \emph{sketch} aberta vá em ferramentas e verifique se a porta selecionada é a porta que o ARDUINO® UNO está conectado.
...
9º Passo: Após ter verificado a porta e estando tudo correto clique no segundo botão da esquerda para direita na parte superior da \emph{sketch} para carregar a programação, o botão tem o formato de uma ceta.
...
11º Passo: Após ter dado o curto na placa  vamos abrir a pasta UnoJoy, abrir novamente a pasta UnoJoy,  agora abra a pasta UnoJoy-\emph{master}, localize e abra a pasta UnoJoy e execute o arquivo \emph{TurnIntoAJoystick}.
...
Obs.: Se der erro de dll ou outro qualquer ao executar o arquivo, será necessário atualizar o \emph{driver}, faça o seguinte para atualizar o \emph{driver}. 

Vamos em painel de controle, \emph{hardware} e sons e dispositivos e impressoras, e clicamos com o botão direito do \emph{mouse} no dispositivo representante ao ARDUINO® UNO que ficou como Dispositivo Desconhecido após o curto, após clicar com o botão direito do \emph{mouse} sobre o dispositivo que representa o Arduino Uno clicamos  em propriedades.
...
Após ter clicado em propriedades, clique em \emph{hardware} e em propriedades novamente.
...
Na próxima janela clique em \emph{Driver} e Atualizar \emph{Driver}, e em Procurar \emph{software} de \emph{driver} no computador.
...
Agora clique em avançar e aguarde finalizar a instalação do \emph{driver}, deverá ficar como na imagem abaixo.
...
Agora execute o arquivo\emph{ TurnIntoAJoystick} novamente e aguarde, o ARDUINO® UNO deve se desconectar do Pc.
...
12º Passo: Retire o cabo USB que liga o ARDUINO® UNO ao Pc e conecte novamente, para certificar que tudo deu certo vá em painel de controle, \emph{hardware} e sons e dispositivos e impressoras e verifique se o ARDUINO® UNO estar como \emph{Joystick}.
...
Para reverter o processo basta dar o curto novamente na placa ARDUINO® UNO, ir na pasta UnoJoy, abrir novamente a pasta UnoJoy, abrir a pasta UnoJoy-\emph{master}, localizar e abrir a pasta UnoJoy e executar o arquivo \emph{TurnIntoAnArduino}, desconectar e conectar novamente o ARDUINO® UNO ao Pc e o processo será revertido.

\emph{Link} para \emph{download} dos arquivos necessário

https://mega.nz/\#!xIhUxC6J!09gFgfu2v187Nu05q7ybW0L6-yoPQdfkY8Axtxx77Xg

\emph{Link} para \emph{download} do \emph{sofware} do ARDUINO® 

https://www.arduino.cc/en/\emph{Main}/\emph{Software}
